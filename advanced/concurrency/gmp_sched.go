...
// 将 GOMAXPROCS 人为限制为 2，却同时运行 8 个 CPU-bound goroutine，导致 Go 调度器频繁进行抢占、让出和重新绑定 P 与 M
// gmpDemo 演示通过调整 GOMAXPROCS 与 GODEBUG 查看 GMP 调度信息。
// GODEBUG=schedtrace=10,scheddetail=1 go run 06-gmp.go 2> sched.logs
func gmpDemo() {
	...
	fmt.Printf("\n所有任务完成, 耗时: %s\n", elapsed)
	fmt.Println("提示：运行时可配合命令 `GODEBUG=schedtrace=10,scheddetail=1 go run 06-gmp.go` 观察调度日志。")
